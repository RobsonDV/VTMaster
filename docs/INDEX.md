...
> Versão **5.5.43** — Atualizado em **02/06/2026** — Autostart ignora blocos vazios/placeholders (só considera blocos com conteúdo real). Inclui v5.5.42 ("Regenerar do zero"), v5.5.41 (dia sempre fresco no startup) e v5.5.40 (Autostart, Stop/Pause preservam o input no ar, disparo de comercial correto, retomada global)
...
| **v5.5.35** | 26/05/2026 | **Relógio voltou pra trás**: guard `sessionStartRef` evita re-disparo quando NTP/DST recua o clock. **Regenerar AutoProg**: correção no merge de blocos comerciais já disparados. **Input trava no AdBreaks**: fix no ciclo de vida do input do primeiro item do bloco. |
| **v5.5.36** | 26/05/2026 | **Scheduler comercial robusto**: `firedCommercialTimesRef` (Set<string> persistido no localStorage por dia via `spotmaster_fired_YYYY-MM-DD`) elimina disparos duplos após crash/reinício; guard `abortRef` antes do disparo; mutex `schedulerFiringRef`; `catchUpGraceMinutes` dinâmico (settings); `cleanupOldFiredKeys` apenas no startup + virada de dia. **Widget de diagnóstico**: `SchedulerLogWidget` — painel colapsável com últimos 60 eventos (info/warn/error), exportação .txt. **Hardening de produção**: I/O async em `scan-music-folder`/`reconcile-music-folders` (`Promise.all` + `readdir`); retry de duração limitado a 3 tentativas/item; botão vMix com estado "Conectando..." + timeout 8s; items `playing` → `pending` no startup; 22 `console.log` guardados com `isDev`; `SCHEMA_VERSION = 1` com verificação no loadAll. |
| **v5.5.37** | 26/05/2026 | **Hotfix crítico Autoplay Comercial**: corrige dois bugs encadeados que impediam o app de iniciar a programação do zero quando idle. (1) FAILSAFE injetava itens via `dispatch()` mas `stateRef.current` ainda estava stale (dispatch é assíncrono) — `stateRef.current` agora é atualizado imediatamente antes do dispatch, mesmo padrão de `startScheduleFromNow`. (2) `startSchedule()` substituído por `startScheduleFromItem(firstItem.id)` no caso idle — inicia exatamente do primeiro item do bloco, marcando tudo anterior como `skipped`. `firedCommercialTimesRef.add()` movido para APÓS a tentativa de iniciar — sem mais bloqueio permanente por tentativa frustrada. |
| **v5.5.38** | 27/05/2026 | **Scheduler: bloco antigo não bloqueia mais blocos posteriores**: quando o guard de session start bloqueia um `triggerTime` stale (bloco muito antigo sem arquivo, ex: AudioOff 19:08 que nunca tocou), o horário é adicionado a `firedCommercialTimesRef` antes de retornar. No próximo tick (1s depois) o scheduler pula o horário stale e chega ao bloco válido (ex: 19:35). Sem este fix, um único bloco pendente antigo bloqueava TODOS os blocos do restante do dia. |
| **v5.5.39** | 31/05/2026 | **Guard de reentrância no `runSequence`**: `runSequenceActiveRef` (ref síncrono) impede que dois loops de playout rodem concorrentes quando os schedulers musical e comercial coincidem na mesma janela de 1s — fim do `Cut` duplicado / inputs fantasma por corrida de scheduler. |
| **v5.5.40** | 02/06/2026 | **Autostart** (toggle independente, cold-start no horário do bloco com janela de 5 min; dono único do início a partir do idle, sem corrida com o autoplaycomercial; countdown na StatusBar). **Stop e Pause preservam o input no ar** (full-sweep do `runSequence`/`stopPlayback` exclui o GUID em PGM — vMix não pula para input aleatório). **Disparo de comercial correto** (`loadNewInput` serializado por mutex `loadLockRef` + `pollForNewInput` casa o input novo por `filePath`). **Retomada de sessão global** (snapshot casado por `filePath` aceitando `pending`; banner `ResumeBanner` global no `App.tsx`, visível em qualquer aba). |
| **v5.5.41** | 02/06/2026 | **Programação do Dia não abre mais "já executada"**: o startup só gerava se a data não existisse e o `loadAll` só resetava `playing` — status `done`/`skipped` de outro dia persistiam e o motor pulava os blocos. Novo marcador `localStorage['vtmaster_last_active_date']` + heurística de carryover (há executado mas 0 veiculação no playLog do dia) regeneram a programação do zero (tudo `pending`) e limpam o fired set quando o dia muda. Same-day reopen com veiculação real preserva o estado (resume intacto). Midnight watcher também grava o marcador. |
| **v5.5.42** | 02/06/2026 | **Botão "Regenerar do zero"** na Programação do Dia: nova ação `regenerateScheduleFresh(date)` no `AppContext` faz REPLACE (tudo `pending`) + limpa o fired set comercial do dia (`spotmaster_fired_<data>` + ref em memória se for hoje). Corrige na hora um dia que abriu já executado — "Atualizar"/"Regerar" (merge) preservam `done`/`skipped` de propósito e não limpavam. Botão com confirmação, bloqueado se a sequência estiver tocando. |
| **v5.5.43** | 02/06/2026 | **Autostart ignora blocos vazios**: o scheduler do Autostart e o countdown da StatusBar consideravam qualquer item `pending` com `scheduledTime`, incluindo placeholders vazios (slots musicais não preenchidos) e pausas isoladas — marcava, por ex., um bloco 07:30 sem conteúdo. Agora exigem conteúdo real (`filePath \|\| inputName \|\| vmix_action`, excluindo `pause`/placeholder), apontando o gatilho/countdown para o primeiro bloco com material. |
